/*********************************************************************
 ...
 历史修改记录:
    ...
    未解决bug：
    1.左对齐返回值及格式化不正确，("%-5s","hhh")。（无符号有符号比较bug，已解决）
    2.浮点数格式化存在破坏堆栈bug。
    3.浮点数格式化在linux下结果不正确bug
**********************************************************************/
...
size_t CISigStr_Strnlen(const char *s, size_t count)
{
    const char *p = s;
    while (*p && count-- > 0)
        p++;
...
int CISigStr_Snprintf(char *str, size_t n, const char *format, ...)
{
    va_list args;
    int ret;
    
    va_start(args, format);
    ret = CISigStr_Vsnprintf(str, n, format, args);
    va_end(args);
    return ret;
}
...
int CISigStr_Vsnprintf(char *str, size_t nmax, const char *format, va_list ap)
{
...
int pvsnfmt_char(char **pinsertion, size_t *nmax, const char UNUSED(fmt), int UNUSED(flags), 
                 int32_t UNUSED(width), int UNUSED(precision), char UNUSED(prefix), va_list *ap)
{
    if (*nmax > 1)
    {
...
int pvsnfmt_str(char **pinsertion, size_t *nmax, const char UNUSED(fmt), int flags, 
                int32_t width, int precision, char UNUSED(prefix), va_list *ap)
{
    ...
    if (precision < 0)
        ...
        len = CISigStr_Strnlen(str, precision);
...
#define POP() \
    *(--stackpos)
    ...
    if (fmt == 'o' && (flags & FLAG_HASH) && precision <= (int32_t)len &&
        precision != 0 && !iszero )
    {
        precision = len + 1;
    }
    ...
    if (*nmax <= 1)
        len = 0;
    //TODO BUG 有符号与无符号转换的bug
    else if (*nmax + 1 < (unsigned)len)
        len = *nmax - 1;
                ...
                int32_t width, int precision, char UNUSED(prefix), va_list *ap)
{
    ...
    if (fmt == 'f')
    {
        ...
        if (leadingzeros)
        {
            ...
            if (*nmax <= 1)
                leadingzeros = 0;
            /*BUG TODO 强制转换可能存在数据丢失*/
            else if (*nmax /* - 1 */ < (unsigned)leadingzeros /* -1 */)
                leadingzeros = *nmax; /* -1 */
        ...
        if (dec > 0)
        {
            if (*nmax <= 1)
                printdigits = 0;
            /*TODO BUG强制转换可能存在数据丢失*/
            else if (*nmax - 1 < (unsigned)dec)
                printdigits = *nmax - 1;
            else
                printdigits = dec;
